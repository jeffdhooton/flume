...
// spawnDaemon forks the current flume binary as a detached background process
// running `flume start --foreground`.
func spawnDaemon(extraArgs ...string) error {
	...
	args := append([]string{"start", "--foreground"}, extraArgs...)
...
func waitForSocket(socketPath string, budget time.Duration) error {
	...
	for {
		if pingDaemonSocket(socketPath) {
			return nil
		}
...
func pingDaemonSocket(path string) bool {
	c, err := rpc.Dial(path)
	if err != nil {
		return false
	}
...
// callDaemon is a one-shot helper: dial, send one request, decode, close.
...
func readPID(layout daemon.Layout) int {
